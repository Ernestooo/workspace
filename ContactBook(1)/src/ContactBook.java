...
public class ContactBook {
	...
	public void setPhone(String name, int phone){
		contact[searchIndex(name)].setPhone(phone);
	}
	...
	private int findPositionFor(Contact c){
		int candidatePosition = 0;
		boolean found = false;
		while (candidatePosition < counter && !found){
			if ((c.compareTo(contact[candidatePosition] )> 0)){
				candidatePosition++;
			}
			else found = true;
		}
